   ...
   draw_sprite(XBuf,OSD_MESSAGE_SPR,screen_blit_x+8,min(io.screen_h,vheight)-16); 
     ...
     for (dum=0;dum<io.screen_h-1;dum++)
    {
      ...
      _eagle (
            ...
            ,(int)EAGLE_buf->line[1+(dum<<1)] // + (( ( (x+1)<<1) * piccy->w ) << 1)       // 2nd eagled line = 320x2 = 640 pixels
           );
    }
...
{
  if (vmode==5)
    return !osd_gfx_set_hugo_mode(GFX_SVGALIB,800,600);

  if (io.screen_w==40*8)
    {
      if (!osd_gfx_set_hugo_mode(GFX_SVGALIB,320,240))
        return 1;
    }

  if (io.screen_w>40*8)
    {
      if (!osd_gfx_set_hugo_mode(GFX_SVGALIB,360,240))
        return 1;
    }

  if ((vmode==0) && (!osd_gfx_set_hugo_mode(GFX_VGA,256,224)));
   else
  if ((vmode<=1) && (!osd_gfx_set_hugo_mode(GFX_SVGALIB,256,240)));
   else
  if ((vmode<=2) && (!osd_gfx_set_hugo_mode(GFX_SVGALIB,256,256)));
   else
  if ((vmode<=3) && (!osd_gfx_set_hugo_mode(GFX_SVGALIB,320,240)));
   else
  if ((vmode<=4) && (!osd_gfx_set_hugo_mode(GFX_SVGALIB,320,200)));
   else
          return 0;
