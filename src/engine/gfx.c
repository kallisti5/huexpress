...
save_gfx_context(int slot_number)
{
	...
	if (slot_number >= MAX_GFX_CONTEXT_SLOT_NUMBER) {
		Log("Internal error in %s(%s), slot %d >= %d\n", __FUNCTION__, __FILE__,
			slot_number, MAX_GFX_CONTEXT_SLOT_NUMBER);
		return;
	}
...
void
dump_uyvy_frame(char* output_buffer)
{
	int x,y;
	uchar* xbuf_pointer;
...
//! Raw dump the current frame into the buffer, for video output purposes
void
dump_yyuv_frame(char* output_buffer)
{
	int x,y;
	uchar* xbuf_pointer;
...
//! Raw dump the current frame into the buffer, for video output purposes
void
dump_rgb_frame(char* output_buffer)
{
	int x,y;
	uchar* xbuf_pointer;
...
//! Raw dump the current frame into the buffer, for video output purposes
void
dump_raw_frame(char* output_buffer)
{
	int y;
	uchar* xbuf_pointer;
	...
	for (y = 0; y < (io.screen_h & 0xFFFE); y++, xbuf_pointer += XBUF_WIDTH,
		output_buffer += (io.screen_w & 0xFFFE)) {
...
void
dump_uyv_frame_separated(char* output_buffer_u, char* output_buffer_y, char* output_buffer_v)
{
	int x,y;
	uchar* xbuf_pointer;
...
dump_video_frame()
{
	...
	if (tmp_video_output_file != NULL) {
		fwrite(frame_buffer_u, (io.screen_w & 0xFFFE) * (io.screen_h & 0xFFFE),
			1, tmp_video_output_file);
...
dump_video_frame()
{
	...
	if ((old_screen_w != (uint16)(io.screen_w & ~1))
...
stop_dump_video()
{
	...
	snprintf(finish_message, 100, "Video dump finished, %d frames dumped, "
		"%d frames compressed (%.2f%% compression)", video_frame_total,
		video_frame_skipped, (video_frame_total != 0 ? 100.0
			* video_frame_skipped / video_frame_total : 100));
