...
follow_BBRi(uint16 where)
{
	...
	// jump
	return where + addr_info_debug[optable_debug[op].addr_mode].size
		+ (char) get_8bit_addr(where + size - 1);
...
follow_BCC(uint16 where)
{
	...
	// jump
	return where + size + (char) get_8bit_addr(where + size - 1);
}
...
follow_JMPabs(uint16 where)
{
	...
	return get_8bit_addr(where + size - 2) + 256 * get_8bit_addr(where +
																 size - 1);
...
follow_JMPindir(uint16 where)
{
	...
	uint16 indir =
		get_8bit_addr(where + size - 2) + 256 * get_8bit_addr(where +
															  size - 1);
