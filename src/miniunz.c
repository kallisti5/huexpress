...
/*!
 * do_extract_currentfile_in_memory : Extract the current pointed file in the
 * 	zip archive into a buffer
 * @param uf the archive file handle
 * @param unzipped_size a pointer to the size of the unzipped data, it is filled
 *	by this function for caller
 * @return uncompressed data as pointer, to be freed by caller or NULL in
 *	case of problem
 */
char *
