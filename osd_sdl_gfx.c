...
int osd_gfx_init(void)
{
	...
	if ((physical_screen = SDL_SetVideoMode(option.want_fullscreen
	...
	if (option.want_hardware_scaling)
	{
		...
		if (option.wanted_hardware_format != 0) {
			...
			olay = SDL_CreateYUVOverlay(fake_io_screen_w, fake_io_screen_h,
				option.wanted_hardware_format, physical_screen);
			if (olay->hw_overlay == 1)
				MESSAGE_INFO("SDL: Hardware scaling enabled using overlay %x.\n",
		...
		if ((option.wanted_hardware_format == 0) || (olay->hw_overlay != 1)) {
			if (((olay = SDL_CreateYUVOverlay(fake_io_screen_w,
				fake_io_screen_h, SDL_UYVY_OVERLAY, physical_screen)) == NULL)
				|| (olay->hw_overlay != 1)) {
				if (((olay = SDL_CreateYUVOverlay(fake_io_screen_w,
					fake_io_screen_h, SDL_YUY2_OVERLAY, physical_screen))
					== NULL) || (olay->hw_overlay != 1)) {
						...
						if ((physical_screen
							...
							? option.fullscreen_width : fake_io_screen_w
								* option.window_size,
					...
					MESSAGE_INFO("SDL: Hardware scaling enabled using YUY2.\n");
			} else
				MESSAGE_INFO("SDL: Hardware scaling enabled using UYVY.\n");
		} else
			host.video.hardware_scaling = FALSE;
	}
